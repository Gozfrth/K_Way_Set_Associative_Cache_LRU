...
class Kway<T>::KImpl::Set{
	public:
		...
		Set(int max_size, int block_size, int num_sets_): set_max_size_(max_size), blk_size_(block_size), num_sets(num_sets_), size_(0), head_(NULL), tail_(NULL), hit_count_(0), miss_count_(0){
		};
		...
		void PutData(T* key){
			...
			memcpy(&newCacheLine->data, (key-blockOffset), blk_size_);
